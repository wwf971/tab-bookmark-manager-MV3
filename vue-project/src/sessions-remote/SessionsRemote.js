...
import { useSettingStore } from '@/settings/Settings'
import { storeToRefs } from 'pinia'
import {
  removeTabRemote,
  uploadTabToServer as _uploadTabToServer,
  uploadTabsToRemote,
  fetchTabsRemoteFromServer as _fetchTabsRemoteFromServer,
  searchTabsRemoteByTags as _searchTabsRemoteByTags
} from './SessionsRemoteRequest.js'
...
export const useTabsRemote = defineStore('tabsRemote', () => {
  ...
  watch(settingsComputed, (newVal) => {
    ...
    console.log(`SessionsRemote.vue: fetchAllRemoteTabsOnInit setting changed to: ${fetchAllRemoteTabsOnInitNew}`)
  ...
  const uploadTabToServer = async ({tab, fetchTabsRemoteRecent = true, options = {}}) => {
    const serverStore = useServerStore()
    const { task = 'create_tab_remote', tags_name = [], tags_id = [], comment = ''} = options
    const urlUploadResult = serverStore.getServerEndpoint(`/url_pool/${task}`)
    if (!urlUploadResult.is_success) {
      return urlUploadResult
    }
    const response = await _uploadTabToServer(tab, options)
  ...
  const fetchTabsRemoteRecent = async (num = 50) => {
    try {
      ...
      const response = await axiosInstance.post(serverUrl, {
        task: 'get_recent_tabs_remote_id',
        limit: num
      })
