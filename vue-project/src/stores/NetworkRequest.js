...
export const useNetworkRequest = defineStore('networkRequest', ()=>{
  ...
  const setTagsForTabRemote = async (url, tags_name_new) => {
    const urlCurrent = serverStore.getUrlCurrent()
    if (!urlCurrent) {
      return {
        is_success: false,
    ...
    try {
      const response = await axiosInstance.post(`${urlCurrent.url}/url_pool/set_tags_for_tab_remote`, {
      ...
      console.error('NetworkRequest.js: set_tags_for_tab_remote error:', error)
      return {
        is_success: false,
