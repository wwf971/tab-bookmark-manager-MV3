  ...
  <div class="container">
    ...
    <div class="upload-buttons">
      ...
      <button 
        class="cache-button" 
        :disabled="isUploadButtonsDisabled"
        @click="requestUploadTabs('url_cache')"
      >
        Upload {{ uploadTabsNum }} Tab(s) As UrlCache
      </button>
      <button 
        class="cache-button" 
...
const fetchSimilarTabs = async (url, title) => {
  ...
  try {
    const result = await networkRequest.getSimilarUrl(url, title)
...
const updateExistingNote = async (noteId, updateDict) => {
  try {
    const result = await networkRequest.updateNote(noteId, updateDict)
    return result
  } catch (error) {
