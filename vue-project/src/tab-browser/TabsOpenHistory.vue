...
const getActiveAndSelectedTabs = computed(() => {
  // console.log('TabsOpenHistory.vue: getActiveAndSelectedTabs computed executing')
  ...
  return (windowId) => {
    ...
    if (cache.has(windowId)) {
      ...
      if (cached.activeLength === currentActive.length && 
        ...
        // console.log(`TabsOpenHistory.vue: Using cached result for window ${windowId}`)
        return cached.result
      }
    }
    
    // console.log(`TabsOpenHistory.vue: Computing for window ${windowId}`)
    ...
    // console.log(`TabsOpenHistory.vue: Window ${windowId} - active: ${activeTabs.length}, selected: ${selectedTabs.length}, combined: ${uniqueTabs.length}`)
    return uniqueTabs
  }
})
...
const getRecentTabs = computed(() => {
  ...
  return (windowId) => {
    ...
    // console.log('TabsOpenHistory.vue: getRecentTabs', recentTabs)
...
const handleActiveOrSelectedTabClick = async (tab) => {
  // console.log('TabsOpenHistory.vue: handleActiveOrSelectedTabClick', tab.id)
  try {
...
const showContextMenu = (event, tab) => {
  console.log('TabsOpenHistory.vue: showContextMenu. tab:', tab)
  emit('show-context-menu', { event, tab })
}

// Lifecycle hooks with detailed logging
onMounted(() => {
  // console.log('TabsOpenHistory.vue: Component mounted')
})

onUnmounted(() => {
  // console.log('TabsOpenHistory.vue: Component unmounted')
})

onUpdated(() => {
  // console.log('TabsOpenHistory.vue: Component updated')
})

// Watch for changes to debug re-renders
// watch(tabsOpenHistory, (newHistory, oldHistory) => {
//   console.log('TabsOpenHistory.vue: tabsOpenHistory changed', {
...
console.log('TabsOpenHistory.vue: Script setup completed')
</script>
