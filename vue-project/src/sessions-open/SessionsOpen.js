...
export const useSessionsOpen = defineStore('tabsOpen', () => {
  ...
  // fast lookup map for tab objects
  const tabsMapOpen = ref(new Map()) // tabId -> { tab, windowId }
  ...
  const updateSessionsOpenHistory = (windowId, tabId) => {
    // console.log('updateSessionsOpenHistory()')
    if (!tabsOpenHistory.value.has(windowId)) {
      tabsOpenHistory.value.set(windowId, { active: [], selected: [], recent: [] })
    }
  ...
  const removeTabFromSessionsOpenHistory = (tabId) => {
  ...
  const onTabOpenAttached = (tabId, winIdNew, indexNew, removeFromWinOld = false) => {
    ...
    updateSessionsOpenHistory(winIdNew, tabId)
  ...
  const initSessionsOpenHistoryFromBackground = async () => {
    if(process.env.VUE_APP_IS_DEV){
      console.log("initSessionsOpenHistoryFromBackground(). process.env.VUE_APP_IS_DEV:", process.env.VUE_APP_IS_DEV)
      return;
    }

    try {
      console.log('initSessionsOpenHistoryFromBackground(): Starting sync...')
      const response = await chrome.runtime.sendMessage({
        action: "getRecentTabs"
      })
      // console.log('initSessionsOpenHistoryFromBackground(): Response from background:', response)
      ...
      if (response.tabsRecentAllWin) {
        ...
        for (const [windowId, tabsRecentId] of Object.entries(response.tabsRecentAllWin)) {
          // console.log('initSessionsOpenHistoryFromBackground(): windowId:', windowId, 'tabsRecentId:', tabsRecentId)
          let tabs = [];
          tabsRecentId.forEach(tabId => {
            console.log('initSessionsOpenHistoryFromBackground(): tabId:', tabId)
            const tabInfo = tabsMapOpen.value.get(tabId)
            if(tabInfo){
              tabs.push(tabInfo.tab)
            }else{
              console.error('initSessionsOpenHistoryFromBackground(): tab not found for tabId:', tabId)
            }
          })
          console.log('initSessionsOpenHistoryFromBackground(): tabs:', tabs)
        ...
        // console.log('initSessionsOpenHistoryFromBackground(): Synced initial recent tabs from background:', response.tabsRecentAllWin)
      } else {
        console.error('initSessionsOpenHistoryFromBackground(): No recent tabs data in response')
      }
    } catch (error) {
  ...
  const initEventListeners = () => {
    ...
    chrome.tabs.onRemoved.addListener((tabId, removeInfo) => {
      ...
      import('@/search/TabsSearch').then(({ useTabsSearch }) => {
      ...
      // remove from history
      removeTabFromSessionsOpenHistory(tabId)
      // remove from sessionsOpen
  ...
  const initTabsOpen = async () => {
    ...
    await initSessionsOpenHistoryFromBackground() // get recent tabs from background.js
  ...
  const removeTabFromWindowsOpen = (tabId, windowId) => {
    ...
    // Window positions are now handled by the component
