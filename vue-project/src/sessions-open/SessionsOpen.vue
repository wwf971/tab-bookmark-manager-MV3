<template>
  <div class="tabs-open-container" :class="{ 'search-mode': isSearchMode }">
    <!-- Header with stats -->
...
import { useServerStore } from '@/network/Server'
...
const winIdVisible = ref(null) // currently visible window id
const scrollEventTimeout = ref(null) // throttling timeout
const windowPosY = ref(new Map()) // windowId -> { top: number, height: number }
...
const handleContextMenuAction = async (event) => {
  ...
  try {
    switch (action) {
      ...
      default:
        console.warn('SessionsOpen.vue: Unknown context menu action:', action)
    }
  } catch (error) {
    console.error('SessionsOpen.vue: Context menu action error:', error)
  } finally {
...
const { handleGridClick, handleGridDoubleClick, handleGridContextMenu } = createGridEventHandlers({
  ...
  onTabRightClick: (event, tab, tabEl) => {
    console.log('SessionsOpen.vue: onTabRightClick() tab:', tab)
...
const getVisibleWindowId = () => {
  ...
  const scrollContainer = isSearchMode.value 
    ? document.querySelector('.content-body')  // search mode. scroll container is .content-body in TabsMainWindow.vue
    : document.querySelector('.windows-container') // non-search mode. scroll container in .windows-container in this component
  ...
  for (const [windowId, position] of windowPosY.value.entries()) {
  ...
  if (visibleWindowId === null && windowPosY.value.size > 0) {
    const firstEntry = windowPosY.value.entries().next().value
...
const onScrollEvent = () => {
  ...
  scrollEventTimeout.value = setTimeout(() => {
    ...
    if (newVisibleWindowId !== winIdVisible.value) {
      winIdVisible.value = newVisibleWindowId
...
const updateTabsMapOpen = () => {
  // console.log('SessionsOpen.vue: updateTabsMapOpen()')
...
onUnmounted(() => {
  // console.log('SessionsOpen.vue: Component unmounted')
})

onUpdated(() => {
  // console.log('SessionsOpen.vue: Component updated')
})

watch(windowCurrentId, (newId, oldId) => {
  console.log('SessionsOpen.vue: windowCurrentId changed', { 
    newId, 
    oldId,
...
// window position tracking for scroll monitoring (component-specific)
const updateWinPos = () => {
...
const scrollToWindow = (windowId) => {
  ...
  const scrollContainer = isSearchMode.value 
    ...
    : document.querySelector('.windows-container')
  if (!scrollContainer) {
    console.warn('scrollToWindow(): Scroll container not found')
    return
  }
...
/* Search mode: remove scroll from windows-container */
.tabs-open-container.search-mode .windows-container {
  overflow-y: visible;
  height: auto;
